...
public class SysIdRoutineLog {
  ...
  public final class MotorLog {
    ...
    public MotorLog value(String name, double value, String unit) {
      // Preserve previous naming convention: "field-motorName-logName"
      String fullName = name + "-" + m_motorName + "-" + m_logName;

      // KoalaLog API: log(name, double value, boolean post)
      // We choose post=false here (no dashboard telemetry). Change if needed.
      KoalaLog.log(fullName, value, false, unit);
      return this;
    }
  ...
  public void recordState(State state) {
    KoalaLog.log("sysid-test-state-" + m_logName, state.toString(), true);
  }
}
