...
/**
 * This is the Pinpoint class. This class extends the Localizer superclass and is a localizer that
 * uses the two wheel odometry set up with the IMU to have more accurate heading readings. The
 * diagram below, which is modified from Road Runner, shows a typical set up.
 */
public class PinpointVisionLocalizer implements Localizer {
  ...
  private final NavigableMap<Double, PinpointVisionLocalizer.VisionUpdate> m_visionUpdates =
      new TreeMap<>();
   ...
   */
  public PinpointVisionLocalizer(HardwareMap map, PinpointVisionConstants constants) {
   ...
   */
  @SuppressLint("NewApi")
  public PinpointVisionLocalizer(
      HardwareMap map, PinpointVisionConstants constants, Pose setStartPose) { // TODO
    ...
    setOdometryStdDevs(
        constants.odometryStdDevs[0], constants.odometryStdDevs[1], constants.odometryStdDevs[2]);
    setVisionMeasurementStdDevs(
        constants.visionMeasurementStdDevs[0],
        constants.visionMeasurementStdDevs[1],
        constants.visionMeasurementStdDevs[2]);
  }
  ...
  /**
   * This returns the current pose estimate. !!! THIS IS THE PINPOINT POSE WITHOUT THE VISION USE
   * getPose for getting robot pose
   *
   ...
   * and due to issues with the PinpointVisionLocalizer, this is functionally the same as
   * setPose(Pose).
   *
  ...
  /**
   * This sets the current pose estimate. Changing this should just change the robot's current pose
   * estimate, not anything to do with the start pose.
   *
  ...
  public void update() {
    odo.update();
    Pose currentPinpointPose =
        PoseConverter.pose2DToPose(odo.getPosition(), PedroCoordinates.INSTANCE);
    totalHeading +=
        MathFunctions.getSmallestAngleDifference(currentPinpointPose.getHeading(), previousHeading);
    ...
    if (m_visionUpdates.isEmpty()) {
      ...
      PinpointVisionLocalizer.VisionUpdate latestVU =
  ...
  /**
   * This returns how far the robot has turned in radians, in a number not clamped between 0 and 2 *
   * pi radians. This is used for some tuning things and nothing actually within the following.
   *
   ...
   * @param yOffset How far forward from the center of the robot is the y-pod? Use positive values
   *     if it's forward and negative if it's to the back.
  ...
  /**
   * This recalibrates the Pinpoint. It will take 0.25 seconds to recalibrate, and the robot must be
   * still
   */
  ...
  /**
   * Return the odometry-only pose at given timestamp (seconds) by linear interpolation. Returns
  ...
  /**
   * Add a vision measurement (pose) with a timestampSeconds (seconds epoch). This will: - reject if
   * measurement too old for buffer, - compute the difference between vision pose and current
   * vision-compensated pose at that time, - scale it by a per-axis Kalman-like gain, - store a
   * VisionUpdate and update the current pose estimate. Usage: when your vision system reports a
   * pose, call: PinpointVisionLocalizer.addVisionMeasurement(visionPose, System.nanoTime()/1e9);
   */
  public void addVisionMeasurement(Pose visionRobotPoseMeters, double timestampSeconds) {
    ...
    double dtheta =
        Math.copySign(
            MathFunctions.getSmallestAngleDifference(
                visionSample.getHeading(), visionRobotPoseMeters.getHeading()),
            raw);
    ...
    double wrappedAngle =
        Math.atan2(Math.sin(currentUnwrappedHeading), Math.cos(currentUnwrappedHeading));

    // Apply correction to visionSample in field coords
    Pose correctedVisionPose =
        new Pose(visionSample.getX() + corr_x_f, visionSample.getY() + corr_y_f, wrappedAngle);

    // Step 7: record the vision update and remove later vision measurements
    PinpointVisionLocalizer.VisionUpdate visionUpdate =
        new PinpointVisionLocalizer.VisionUpdate(correctedVisionPose, odomSample);
    m_visionUpdates.put(timestampSeconds, visionUpdate);
    m_visionUpdates.tailMap(timestampSeconds, false).clear();
    KoalaLog.log(
        "vision/X",
        new double[] {
          visionUpdate.visionPose.getX(),
          visionUpdate.visionPose.getY(),
          visionUpdate.visionPose.getHeading()
        },
        true);
