...
public class FusionLocalizer implements Localizer {
  ...
  public FusionLocalizer(
    ...
    this.R =
        MatrixUtil.diag(measurementVariance[0], measurementVariance[1], measurementVariance[2]);
  ...
  public void update() {
    ...
    // Perform twist integration to propagate the fused position estimate based on how the odometry
    // thinks the robot has moved
   ...
   *
   * <p>wₖ is the noise in the system caused by sensor uncertainty, a zero-mean random vector with
   * covariance Q.
   *
   * <p>The Kalman Filter update step is given by:
   *
   * <pre>
   *     Pₖ₊₁ = F * Pₖ * Fᵀ + G * Q * Gᵀ
   * </pre>
   ...
   *
   * <p>The State Transition Matrix F is given by I + ∂f/∂x. We computed our twist integration using
   * a first-order forward-Euler approximation. Therefore, f only depends on the twist, not on x, so
   * ∂f/∂x = 0 and F = I.
   *
   * <p>The Control-to-State Matrix G is given by ∂xₖ₊₁ / ∂wₖ. Here this is simply I * Δt.
   *
   * <p>The Kalman update is Pₖ₊₁ = F * Pₖ * Fᵀ + G * Q * Gᵀ. With F = I and G = I * Δt, we get Pₖ₊₁
   * = Q * Δt².
   *
   ...
   *
   * @param measuredPose the measured position by the camera, enter NaN to a specific axis if the
   *     camera couldn't measure that axis
  ...
  public void addMeasurement(Pose measuredPose, long timestamp) {
              ...
              {measY ? measuredPose.getY() - pastPose.getY() : 0},
              {
                measH
                    ? MathFunctions.normalizeAngle(
                        measuredPose.getHeading() - pastPose.getHeading())
                    : 0
              }
            });

    // Measurement mask M
    Matrix M = MatrixUtil.diag(measX ? 1 : 0, measY ? 1 : 0, measH ? 1 : 0);
    ...
    Matrix updatedCovariance =
        IK.multiply(Pm).multiply(IK.transposed()).plus(K.multiply(R).multiply(K.transposed()));
  ...
  private Matrix invert(Matrix m) {
    if (m.getRows() != m.getColumns()) throw new IllegalStateException("Matrix must be square");
  ...
  private static Pose interpolate(long timestamp, NavigableMap<Long, Pose> history) {
    ...
    double headingDiff =
        MathFunctions.getSmallestAngleDifference(upperPose.getHeading(), lowerPose.getHeading());
  ...
  public void setPose(Pose setPose) {
    ...
    if (poseHistory.lastEntry() != null) poseHistory.lastEntry().setValue(setPose.copy());
