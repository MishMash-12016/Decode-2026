...
/**
 * A subsystem that wraps and manages one or more {@link CuttleServo} instances, allowing for
 * position control and command generation. Supports instant positioning, gradual movement over
 * time, and conditional positioning based on button input.
 */
public class ServoSubsystem extends MMSubsystem {
  ...
  public Command setPositionRunCommand(double position) {
    return new StartEndCommand(() -> setPosition(position), () -> {}, this);
  }
   ...
   */
  public Command setPositionByButtonCommand(
      double defaultPosition, BooleanSupplier button, double buttonPressedPosition) {
  ...
  /**
   * Gets the logical position of the first servo in the list, taking into account offset and
   * direction.
   *
   ...
   */
  public void withServo(String servoName, Direction servoDirection, double offset) {
    CuttleServo servo =
        new CuttleServo(MMRobot.getInstance().currentOpMode.hardwareMap, servoName)
            .setOffset(offset)
   ...
   * @param offset the offset to apply
   */
  public void withServo(
      CuttleRevHub revHub, int portNumber, Direction servoDirection, double offset) {
    CuttleServo servo =
        new CuttleServo(revHub, portNumber).setOffset(offset).setDirection(servoDirection);
    servoList.add(servo);
  }

  public void withDefaultCommand(Command command) {
