package org.firstinspires.ftc.teamcode.Libraries.MMLib.Utils.exterpolation;

import java.util.HashMap;
import java.util.Map;

/**
 * Represents a map of data to exterpolated from to, (Shooter speeds for specific distances, etc.)
 */
public class ExterpolationMap {
  ...
  /**
   * Gets the value for a specific x coordinate, usually the shooter speed for a specific distance
   * from the target.
   *
  ...
  public double exterpolate(double x) {
    ...
    if (x
        > getDataPoints().keySet().stream().max(Double::compare).orElse(Double.POSITIVE_INFINITY)) {
      ...
      return linearExterpolation(
          findLargestKey(getDataPoints()),
          largestValue,
          findSecondLargestKey(getDataPoints()),
          secondLargestValue,
          x);
    }
    ...
    if (x
        < getDataPoints().keySet().stream().min(Double::compare).orElse(Double.NEGATIVE_INFINITY)) {
      ...
      return linearExterpolation(
          findSmallestKey(getDataPoints()),
          smallestValue,
          findSecondSmallestKey(getDataPoints()),
          secondSmallestValue,
          x);
    }
    ...
    for (Map.Entry<Double, Double> point : getDataPoints().entrySet()) {
      ...
      } else if (point.getKey() > x
          && (closestAfter == null || point.getKey() < closestAfter.getX())) {
    ...
    return linearExterpolation(
        closestBefore.getX(),
        closestBefore.getValue(),
        closestAfter.getX(),
        closestAfter.getValue(),
        x);
  }

  /**
   * Linear exterpolation between two 2D points. Finds the Y value between the two points given an X
   * value between the two points {@see https://theeducationlife.com/exterpolation-formula/}
   *
